...
const STATUS_META: Record<string, { cls: string; icon: string; label: string }> = {
  draft:     { cls: 'text-[#6A6470] bg-[#988d9c]/10 border-[#988d9c]/20',  icon: 'draft',        label: 'Draft'     },
    ...
    <div className="p-6 md:p-8 space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <h1 className="font-headline text-3xl font-extrabold tracking-tighter text-[#1C1814]">Posts</h1>
          {meta && <p className="text-[#6A6470] text-sm mt-1">{meta.total.toLocaleString()} total posts</p>}
        </div>
        <button
        ...
        <div className="flex flex-wrap gap-2">
            ...
            return (
              <button
                ...
                className={[
                  ...
                  active ? m.cls : 'text-[#6A6470] bg-transparent border-[#1C1814]/20 hover:border-[#1C1814]/40 hover:text-[#1C1814]',
                ].join(' ')}
              >
          ...
          {status && (
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs text-[#1C1814] border border-[#1C1814]/15 hover:text-[#1C1814] transition-all"
            >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#1C1814]/10">
                <th className="text-left px-5 py-4 text-[#6A6470] text-xs uppercase tracking-widest font-semibold">Platform</th>
                <th className="text-left px-5 py-4 text-[#6A6470] text-xs uppercase tracking-widest font-semibold">Caption</th>
                <th className="text-left px-5 py-4 text-[#6A6470] text-xs uppercase tracking-widest font-semibold">Status</th>
                <th className="text-left px-5 py-4 text-[#6A6470] text-xs uppercase tracking-widest font-semibold">Owner</th>
                <th className="text-left px-5 py-4 text-[#6A6470] text-xs uppercase tracking-widest font-semibold">Workspace</th>
                <th className="text-right px-5 py-4 text-[#6A6470] text-xs uppercase tracking-widest font-semibold">Date</th>
                <th className="text-center px-5 py-4 text-[#6A6470] text-xs uppercase tracking-widest font-semibold">Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i} className="border-b border-[#1C1814]/5 animate-pulse">
                    <td className="px-5 py-4"><div className="h-5 w-20 bg-[#E5DFD6] rounded-lg" /></td>
                    <td className="px-5 py-4"><div className="h-3 w-48 bg-[#E5DFD6] rounded-full" /></td>
                    <td className="px-5 py-4"><div className="h-5 w-24 bg-[#E5DFD6] rounded-lg" /></td>
                    <td className="px-5 py-4"><div className="h-3 w-28 bg-[#E5DFD6] rounded-full" /></td>
                    <td className="px-5 py-4"><div className="h-3 w-24 bg-[#E5DFD6] rounded-full" /></td>
                    <td className="px-5 py-4"><div className="h-3 w-20 bg-[#E5DFD6] rounded-full ml-auto" /></td>
                    <td className="px-5 py-4"><div className="h-6 w-24 bg-[#E5DFD6] rounded-lg mx-auto" /></td>
                  </tr>
                ))
              ) : posts.length === 0 ? (
                <tr>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 max-w-[240px]">
                        {post.caption
                          ? <p className="text-[#5C5650] text-xs truncate">{post.caption}</p>
                          : <p className="text-[#1C1814] text-xs italic">No caption</p>
                        }
                        <p className="text-[#1C1814] text-[10px] mt-0.5 uppercase tracking-wide">{post.post_type}</p>
                      </td>

                      <td className="px-5 py-4"><StatusBadge status={post.status} /></td>

                      <td className="px-5 py-4 text-[#6A6470] text-xs">{post.owner_email}</td>

                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5">
                          <div className="w-4 h-4 rounded bg-gradient-to-tr from-[#7DD3C7]/40 to-[#D6A86A]/40 flex items-center justify-center shrink-0">
                            <span className="material-symbols-outlined text-[#7DD3C7]" style={{ fontSize: 10 }}>workspaces</span>
                          </div>
                          <span className="text-[#5C5650] text-xs">{post.workspace_name}</span>
                        </div>
                      </td>

                      <td className="px-5 py-4 text-right text-[#6A6470] text-xs whitespace-nowrap">
                        {post.published_at
        ...
        {confirm && (
          <div className="p-8">
            ...
            <p className="text-xs text-[#6A6470] mb-1">
              <span className="text-[#5C5650] font-semibold">{confirm.post.owner_email}</span>
              {' · '}
              <span className="uppercase">{confirm.post.platform}</span>
            </p>
            {confirm.post.caption && (
              <p className="text-xs text-[#1C1814] truncate mb-3">{confirm.post.caption}</p>
            )}
            ...
            <div className="flex flex-col gap-2.5">
              <button
                ...
                className={[
                  ...
                  isDeactivate
                    ? 'bg-[#f87171] text-[#1C1814] hover:bg-[#fca5a5]'
