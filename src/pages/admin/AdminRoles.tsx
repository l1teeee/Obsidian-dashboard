  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] p-6 lg:p-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-headline font-extrabold text-[#1C1814] tracking-tight mb-1">Custom Roles</h1>
          <p className="text-sm text-[#6A6470]">Create roles with specific permissions and assign them to users.</p>
        </div>
        <button
          onClick={startCreate}
        ...
        <div className="flex gap-5 items-start">
          ...
          <div className="w-64 shrink-0 flex flex-col gap-2">
            {roles.length === 0 && !isCreating && (
              <div className="rounded-2xl border border-[#1C1814]/15 bg-[#F4F0E8] p-5 text-center">
                <span className="material-symbols-outlined text-[#1C1814] block mb-2" style={{ fontSize: 28 }}>badge</span>
                <p className="text-sm text-[#1C1814]">No roles yet.</p>
                <p className="text-xs text-[#1C1814]/60 mt-1">Click "New Role" to create one.</p>
              </div>
            )}
            {roles.map(role => {
                ...
                <button
                  ...
                  className={[
                    ...
                    active
                      ...
                      : 'border-[#1C1814]/15 bg-[#F4F0E8] hover:border-[#1C1814]/30 hover:bg-white/[0.02]',
                  ].join(' ')}
                >
                  <div className="flex items-center gap-2.5 mb-1">
                    <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: role.color ?? '#6366f1' }} />
                    <span className="text-sm font-semibold text-[#1C1814] truncate">{role.name}</span>
                  </div>
                  <div className="flex items-center justify-between pl-5">
                    <span className="text-xs text-[#1C1814] truncate">{role.description ?? 'No description'}</span>
                    <span className="text-xs text-[#6A6470] shrink-0 ml-2">{role.user_count} users</span>
                  </div>
                </button>
              );
            })}
          ...
          {/* Right: editor panel */}
          {!showPanel ? (
            <div className="flex-1 rounded-2xl border border-[#1C1814]/10 bg-[#F4F0E8] flex flex-col items-center justify-center h-64">
              <span className="material-symbols-outlined text-[#2a2a2a] mb-3" style={{ fontSize: 40 }}>manage_accounts</span>
              <p className="text-sm text-[#1C1814]">Select a role to edit or create a new one.</p>
            </div>
          ) : (
            <div className="flex-1 rounded-2xl border border-[#1C1814]/15 bg-[#F4F0E8] overflow-hidden">
              {/* Panel header */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-[#1C1814]/15">
                <h2 className="text-base font-headline font-bold text-[#1C1814]">
              ...
              <div className="p-6 space-y-6 overflow-y-auto max-h-[calc(100vh-220px)]">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-[#6A6470] mb-2 uppercase tracking-wider">Name</label>
                    <input
                      value={form.name}
                      onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                      placeholder="e.g. Content Manager"
                      className="w-full bg-[#111] border border-[#1C1814]/20 rounded-xl px-3.5 py-2.5 text-sm text-[#1C1814] placeholder-[#1C1814] focus:outline-none focus:border-[#f87171]/40 transition-colors"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#6A6470] mb-2 uppercase tracking-wider">Description</label>
                    <input
                ...
                {/* Color */}
                <div>
                  <label className="block text-xs font-semibold text-[#6A6470] mb-2 uppercase tracking-wider">Color</label>
                ...
                {/* Permissions */}
                <div>
                  <label className="block text-xs font-semibold text-[#6A6470] mb-3 uppercase tracking-wider">Permissions</label>
                  <div className="space-y-3">
                    {groups.map(([category, perms]) => (
                      <div key={category} className="rounded-xl border border-[#1C1814]/12 overflow-hidden">
                        <div className="px-4 py-2 bg-[#111]/50 flex items-center justify-between">
                          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#1C1814]">{category}</span>
                          <button
                            onClick={() => {
                            ...
                            className="text-[10px] text-[#1C1814] hover:text-[#6A6470] transition-colors"
                          >
                            {perms.every(p => form.permissions.includes(p.key)) ? 'Uncheck all' : 'Check all'}
                          </button>
                        </div>
                        <div className="divide-y divide-[#1C1814]/08">
                          {perms.map(perm => {
                                ...
                                <span
                                  ...
                                  className={[
                                    ...
                                    checked
                                      ...
                                      : 'border-[#1C1814]/30 hover:border-[#1C1814]/60',
                                  ].join(' ')}
                                >
                                  {checked && <span className="material-symbols-outlined text-[#f87171]" style={{ fontSize: 11, fontVariationSettings: "'FILL' 1" }}>check</span>}
                                </span>
                                <span className="text-sm text-[#5C5650]">{perm.name}</span>
                                <span className="text-xs text-[#1C1814] ml-auto font-mono">{perm.key}</span>
                              </label>
                            );
                          })}
                        </div>
                ...
                {!isCreating && selectedId && (
                  <div>
                    <label className="block text-xs font-semibold text-[#6A6470] mb-3 uppercase tracking-wider">
                      Assigned Users
                      {roleUsers.length > 0 && <span className="ml-2 text-[#1C1814]">({roleUsers.length})</span>}
                    </label>
                    ...
                    <div className="relative mb-3">
                      <input
                        ...
                        className="w-full bg-[#111] border border-[#1C1814]/20 rounded-xl pl-9 pr-3.5 py-2.5 text-sm text-[#1C1814] placeholder-[#1C1814] focus:outline-none focus:border-[#f87171]/40 transition-colors"
                      />
                      <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#1C1814]" style={{ fontSize: 16 }}>
                        {searching ? 'progress_activity' : 'search'}
                      </span>
                      {searchResults.length > 0 && (
                        <div className="absolute top-full mt-1.5 left-0 right-0 bg-[#161616] border border-[#1C1814]/20 rounded-xl shadow-2xl z-20 overflow-hidden">
                          {searchResults.map(u => (
                            <button
                              key={u.id}
                              onClick={() => { void assignUser(u); }}
                              className="w-full text-left flex items-center gap-3 px-4 py-3 hover:bg-white/[0.04] transition-colors border-b border-[#1C1814]/10 last:border-0"
                            >
                              <div className="w-7 h-7 rounded-full bg-[#1C1814]/20 flex items-center justify-center shrink-0">
                                <span className="text-xs font-bold text-[#6A6470]">{(u.name ?? u.email)[0].toUpperCase()}</span>
                              </div>
                              <div className="min-w-0">
                                <p className="text-sm text-[#1C1814] truncate">{u.name ?? u.email}</p>
                                {u.name && <p className="text-xs text-[#1C1814] truncate">{u.email}</p>}
                              </div>
                              <span className="material-symbols-outlined text-[#1C1814] ml-auto shrink-0" style={{ fontSize: 16 }}>person_add</span>
                            </button>
                          ))}
                        </div>
                      )}
                    ...
                    {loadingUsers ? (
                      <div className="h-10 flex items-center justify-center">
                        <span className="material-symbols-outlined text-[#1C1814] animate-spin" style={{ fontSize: 20 }}>progress_activity</span>
                      </div>
                    ) : roleUsers.length === 0 ? (
                      <p className="text-sm text-[#1C1814] text-center py-4">No users assigned yet.</p>
                    ) : (
                      <div className="space-y-1.5">
                        {roleUsers.map(u => (
                          <div
                            key={u.id}
                            className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-[#111] border border-[#1C1814]/12"
                          >
                            <div className="w-7 h-7 rounded-full bg-[#1C1814]/20 flex items-center justify-center shrink-0">
                              <span className="text-xs font-bold text-[#6A6470]">{(u.name ?? u.email)[0].toUpperCase()}</span>
                            </div>
                            ...
                            <span className={[
                              ...
                              'bg-[#1C1814]/20 text-[#6A6470]',
                            ].join(' ')}>{u.plan || 'starter'}</span>
                            <button
                              onClick={() => { void unassignUser(u.id); }}
                              className="text-[#1C1814] hover:text-[#f87171] transition-colors ml-1"
                            >
                ...
                <div className="flex justify-end pt-2">
                  <button
                    ...
                    disabled={saving}
                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#f87171] text-[#1C1814] text-sm font-bold hover:bg-[#fca5a5] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                  >
      ...
      <Modal open={deleteModal} onClose={() => setDeleteModal(false)} maxWidth="max-w-sm">
        <div className="p-7">
          ...
          <h2 className="text-lg font-headline font-extrabold text-[#1C1814] mb-1">Delete Role?</h2>
          <p className="text-sm text-[#6A6470] mb-6">
            This will delete <strong className="text-[#1C1814]">{selectedRole?.name}</strong> and remove it from all assigned users. This cannot be undone.
          </p>
          <div className="flex flex-col gap-2.5">
            <button onClick={() => { void handleDelete(); }} className="w-full py-3 rounded-xl bg-[#f87171] text-[#1C1814] font-bold text-sm hover:bg-[#fca5a5] transition-all">
              Yes, delete role
            </button>
            <button onClick={() => setDeleteModal(false)} className="w-full py-3 rounded-xl border border-[#1C1814]/20 text-sm font-semibold text-[#5C5650] hover:bg-[#F0EBE2] hover:text-[#1C1814] transition-all">
              Cancel
            </button>
          </div>
        </div>
