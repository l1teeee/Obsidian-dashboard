...
const TOOL_COLORS = ['#111827', '#60a5fa', '#047857', '#f97316', '#f472b6', '#34d399'];

const PLAN_META: Record<string, { label: string; color: string }> = {
  free:       { label: 'Free',       color: '#60a5fa' },
  starter:    { label: 'Starter',    color: '#94A3B8' },
  pro:        { label: 'Pro',        color: '#111827' },
  enterprise: { label: 'Enterprise', color: '#047857' },
};
...
function StatCard({ icon, label, value, sub, color = '#111827' }: {
  ...
  return (
    <div className="rounded-2xl border border-[#0F172A]/15 bg-[#F8FAFC] p-5">
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <span className="text-xs text-[#64748B] font-semibold uppercase tracking-wider">{label}</span>
      </div>
      <p className="text-2xl font-headline font-extrabold text-[#0F172A] tracking-tight">{value}</p>
      {sub && <p className="text-xs text-[#0F172A] mt-1">{sub}</p>}
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-6 lg:p-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap mb-8">
        <div>
          <h1 className="text-2xl font-headline font-extrabold text-[#0F172A] tracking-tight mb-1">Token Usage</h1>
          <p className="text-sm text-[#64748B]">Monitor AI token consumption across all users and tools.</p>
        </div>
        <div className="flex gap-1.5 bg-[#F8FAFC] border border-[#0F172A]/15 rounded-xl p-1">
          {PERIODS.map(p => (
            <button
              key={p.key}
              ...
              className={[
                ...
                period === p.key ? 'bg-[#111827]/15 text-[#111827]' : 'text-[#64748B] hover:text-[#0F172A]',
              ].join(' ')}
            >
              {p.label}
            </button>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {/* Tool breakdown */}
            <div className="rounded-2xl border border-[#0F172A]/15 bg-[#F8FAFC] overflow-hidden">
              <div className="px-5 py-4 border-b border-[#0F172A]/15">
                <h2 className="text-sm font-headline font-bold text-[#0F172A]">Usage by Tool</h2>
                <p className="text-xs text-[#0F172A] mt-0.5">Tokens consumed per AI feature</p>
              </div>
              <div className="p-5 flex flex-col gap-4">
                {tools.length === 0 ? (
                  <p className="text-sm text-[#0F172A] text-center py-6">No AI usage recorded yet.</p>
                ) : tools.map((tool, idx) => {
                    ...
                    <div key={tool.tool}>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-[#0F172A] font-semibold font-headline">{label}</span>
                        </div>
                        <div className="text-right">
                          <span className="text-sm font-bold font-mono" style={{ color }}>{fmtTokens(tool.total_tokens)}</span>
                          <span className="text-xs text-[#0F172A] ml-2">{tool.total_calls} calls</span>
                        </div>
                      </div>
            ...
            <div className="rounded-2xl border border-[#0F172A]/15 bg-[#F8FAFC] overflow-hidden">
              <div className="px-5 py-4 border-b border-[#0F172A]/15">
                <h2 className="text-sm font-headline font-bold text-[#0F172A]">Top Users</h2>
                <p className="text-xs text-[#0F172A] mt-0.5">Highest token consumers this period</p>
              </div>
              {users.length === 0 ? (
                <p className="text-sm text-[#0F172A] text-center py-10">No usage data yet.</p>
              ) : (
                  ...
                  {users.map((u, idx) => {
                    ...
                    return (
                      <div key={u.user_id} className="flex items-center gap-3 px-5 py-3.5">
                        <span className="text-xs text-[#0F172A] w-5 shrink-0 text-right font-mono">{idx + 1}</span>
                        <div className="w-8 h-8 rounded-full bg-[#111827]/15 flex items-center justify-center shrink-0">
                          <span className="text-xs font-bold text-[#111827]">{initial}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-[#0F172A] font-semibold truncate">{u.name ?? u.email}</p>
                          {u.name && <p className="text-[10px] text-[#0F172A] truncate">{u.email}</p>}
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-sm font-bold font-mono text-[#111827]">{fmtTokens(u.total_tokens)}</p>
                          <p className="text-[10px] text-[#0F172A]">{pct}% · {u.total_calls} calls</p>
                        </div>
                      </div>
                    );
                  })}
          ...
          <div className="rounded-2xl border border-[#0F172A]/15 bg-[#F8FAFC] overflow-hidden">
            <div className="px-5 py-4 border-b border-[#0F172A]/15">
              <h2 className="text-sm font-headline font-bold text-[#0F172A]">Monthly Token Limits</h2>
              <p className="text-xs text-[#0F172A] mt-0.5">Set 0 for unlimited. Resets on the 1st of each month.</p>
            </div>
            <div className="divide-y divide-[#1c1b1b]">
              {limits.map(limit => {
                const meta  = PLAN_META[limit.plan] ?? { label: limit.plan, color: '#94A3B8' };
                  ...
                  <div key={limit.plan} className="flex items-center gap-4 px-5 py-4">
                    ...
                    <div className="flex-1 relative">
                      <input
                        ...
                        className="w-full bg-white border border-border rounded-xl px-4 py-2.5 text-sm text-[#0F172A] font-mono focus:outline-none focus:border-[#0F172A]/50 transition-all"
                        placeholder="0"
                      />
                    </div>
                    <div className="text-xs text-[#0F172A] w-28 shrink-0">
