...
function Field({
  ...
  const inputCls = [
    ...
    'focus:ring-2 focus:ring-[#7DD3C7]/20 focus:border-[#7DD3C7] outline-none transition-all',
    'placeholder:text-[#6A6470]/40',
  ...
  return (
    <div className="space-y-1.5">
      ...
      {editing && (
        <p className="text-[11px] text-[#1C1814] leading-relaxed pl-6">{description}</p>
      )}
      ...
      {editing ? (
        ...
        <div className="pl-6">
          {value.trim() ? (
            <p className="text-sm text-[#5C5650] leading-relaxed whitespace-pre-wrap">{value}</p>
          ) : (
    ...
    <div ref={pageRef} className="max-w-2xl mx-auto px-6 py-10 space-y-10">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-[#7DD3C7]/20 to-[#D6A86A]/20 border border-[#7DD3C7]/25 flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-[#7DD3C7]" style={{ fontSize: 20 }}>auto_awesome</span>
            </div>
            <div>
              <h1 className="text-2xl font-headline font-extrabold tracking-tight text-[#1C1814]">AI Settings</h1>
              <p className="text-xs text-[#6A6470]">Context ChatGPT uses when generating your captions</p>
            </div>
          </div>
          {active && (
      ...
      {loading ? (
        <div className="space-y-6">
          {[1,2,3,4].map(i => (
            <div key={i} className="h-24 rounded-2xl bg-[#FAF7F2] animate-pulse" />
          ))}
        </div>
      ) : (
        <>
          {sections.map(section => (
            <section key={section.title} data-ai-section className="space-y-5">
              <div className="flex items-center gap-2 pb-1 border-b border-[#1C1814]/15">
                <span className="material-symbols-outlined text-[#7DD3C7]" style={{ fontSize: 14 }}>{section.icon}</span>
                <h2 className="text-xs font-bold uppercase tracking-widest text-[#6A6470]">{section.title}</h2>
              </div>
