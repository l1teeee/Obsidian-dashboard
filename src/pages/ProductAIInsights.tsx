  ...
  return (
    <ProductShell>
        ...
        <div className="mx-auto max-w-5xl px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="mt-5 text-[clamp(36px,5vw,60px)] font-medium leading-[1.08] tracking-[-0.035em] text-[#0F172A]">
              Turn performance<br />
              <span className="text-[#111827]">into the next best move.</span>
            </h1>
      ...
      <section className="py-12 mx-auto max-w-4xl px-6">
        <div className="rounded-2xl border border-border bg-[#FFFFFF] overflow-hidden">
          <div className="px-6 py-4 border-b border-[rgba(15,23,42,0.08)] flex items-center gap-3">
            <span className="material-symbols-outlined text-[#111827]" style={{ fontSize: 18, fontVariationSettings: "'FILL' 1" }}>auto_awesome</span>
            <span className="text-sm font-bold text-[#0F172A]">Caption assistant</span>
                ...
                <button
                  ...
                  style={
                    activeCap === i
                      ...
                      : { borderColor: 'rgba(15,23,42,0.12)', color: '#64748B' }
                  }
                >
                  {c.label}
                </button>
            ...
            <AnimatePresence mode="wait">
              {!typing && cap && (
                <motion.p
                  ...
                  className="text-sm text-[#0F172A]/80 leading-relaxed"
                >
                  {cap.text}
                </motion.p>
              )}
                ...
                >
                  {[0, 1, 2].map(i => (
                    <motion.div
                      ...
                      className="w-1.5 h-1.5 rounded-full bg-[#111827]"
                    />
                  ))}
                </motion.div>
              )}
          ...
          <div className="px-6 py-3 border-t border-[rgba(15,23,42,0.06)] flex items-center justify-between">
            <span className="text-[10px] text-[#94A3B8]">Generated in 0.8s · optimized for {cap?.label}</span>
            <button className="text-[10px] font-semibold text-[#111827] hover:text-[#0B1220] transition-colors cursor-pointer">Regenerate</button>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 mx-auto max-w-5xl px-6">
        <div className="flex items-center gap-2.5 mb-6">
          ...
          <p className="text-[0.68rem] font-bold uppercase tracking-[0.2em] text-[#111827]">Live recommendations for your account</p>
          <span className="ml-auto text-[10px] text-[#94A3B8] shrink-0">Updated 2 min ago</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Featured: Best time to post */}
          <div className="rounded-2xl border border-border bg-[#FFFFFF] p-6 flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <p className="text-[10px] font-medium text-[#64748B] uppercase tracking-wide">Best time to post</p>
              <span className="px-1.5 py-0.5 rounded-full bg-[#111827]/10 text-[#111827] text-[9px] font-bold">Today</span>
            </div>
            <div className="text-center mb-6">
              <p className="text-[42px] font-bold leading-none text-[#0F172A]">9:15</p>
              <p className="text-base text-[#64748B] mt-1">AM · Tuesday</p>
            </div>
                ...
                {[
                  ...
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div
                      ...
                      style={{ height: `${row.v}%`, backgroundColor: i === 1 ? '#111827' : '#F1F5F9' }}
                    />
                    <span className="text-[8px] text-[#94A3B8]">{row.d}</span>
                  </div>
                ))}
              </div>
              <p className="text-[10px] text-[#94A3B8]">Your audience is most active then</p>
            </div>
          </div>
          ...
          <div className="md:col-span-2 flex flex-col gap-4">
            {/* Hashtag cloud */}
            <div className="rounded-2xl border border-border bg-[#FFFFFF] p-5">
              <div className="flex items-center justify-between mb-3">
                <p className="text-[10px] font-medium text-[#64748B] uppercase tracking-wide">Trending hashtags</p>
                <span className="px-1.5 py-0.5 rounded-full bg-[#111827]/10 text-[#111827] text-[9px] font-bold">Hot</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {['#ContentMarketing', '#SocialMedia', '#Vielinks', '#Creator', '#DigitalMarketing', '#GrowthHacking'].map((tag) => (
                  <span key={tag} className="px-2.5 py-1 rounded-full bg-[#F1F5F9] border border-[rgba(15,23,42,0.08)] text-[11px] font-medium text-[#0F172A]">
                    {tag}
                  </span>
                ))}
              </div>
              <p className="text-[10px] text-[#94A3B8] mt-3">+240% volume this week · #ContentMarketing is breaking out</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="rounded-2xl border border-border bg-[#FFFFFF] p-5">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[10px] font-medium text-[#64748B] uppercase tracking-wide">Caption length</p>
                  <span className="px-1.5 py-0.5 rounded-full bg-[#111827]/10 text-[#111827] text-[9px] font-bold">Tip</span>
                </div>
                <p className="text-2xl font-bold text-[#0F172A]">80-120</p>
                <p className="text-[11px] text-[#64748B] mt-0.5 mb-3">chars for Instagram</p>
                <div className="flex gap-0.5">
                  {Array.from({ length: 20 }).map((_, i) => (
                    <div key={i} className={`h-1.5 flex-1 rounded-full ${i >= 6 && i <= 11 ? 'bg-[#111827]' : 'bg-[#F1F5F9]'}`} />
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-border bg-[#FFFFFF] p-5">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[10px] font-medium text-[#64748B] uppercase tracking-wide">Top format</p>
                  <span className="px-1.5 py-0.5 rounded-full bg-[#111827]/10 text-[#111827] text-[9px] font-bold">Pro</span>
                </div>
                <p className="text-2xl font-bold text-[#0F172A]">Carousel</p>
                <p className="text-[11px] text-[#64748B] mt-0.5 mb-3">3x more saves</p>
                <div className="flex gap-1 items-end h-8">
                  {[30, 60, 100, 45, 20].map((h, i) => (
                    <div key={i} className={`flex-1 rounded-sm ${i === 2 ? 'bg-[#111827]' : 'bg-[#F1F5F9]'}`} style={{ height: `${h}%` }} />
                  ))}
                </div>
              </div>
            </div>
      ...
      <section className="py-16 border-y border-[rgba(15,23,42,0.08)]">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center mb-10">
            <p className="text-[0.68rem] font-bold uppercase tracking-[0.2em] text-[#111827] mb-3">By platform</p>
            <h2 className="text-[clamp(24px,3.5vw,40px)] font-medium tracking-[-0.03em] text-[#0F172A]">Advice tailored to each channel.</h2>
            <p className="mt-3 text-[#64748B] max-w-md mx-auto text-sm leading-relaxed">Instagram, LinkedIn, and Facebook have different audiences and algorithms. Your recommendations reflect that.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-4">
            {PLATFORM_RECS.map((p) => (
              <div key={p.name} className="rounded-2xl border border-border bg-[#FFFFFF] p-6">
      ...
      <section className="py-16 mx-auto max-w-5xl px-6">
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <div>
            <p className="text-[0.68rem] font-bold uppercase tracking-[0.2em] text-[#111827] mb-3">Best-time engine</p>
            <h2 className="text-[clamp(24px,3.5vw,40px)] font-medium tracking-[-0.03em] text-[#0F172A] mb-4">Post when your audience is actually there.</h2>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={row.day} className="flex items-center gap-3">
                  <span className="text-xs text-[#64748B] w-24 shrink-0">{row.day}</span>
                  <span className="text-xs font-bold text-[#0F172A] w-16 shrink-0">{row.time}</span>
                  ...
                  <span className="text-[10px] text-[#94A3B8] w-8 text-right">{row.strength}%</span>
                </div>
              ))}
            </div>
          </div>
          <div className="rounded-2xl border border-border bg-[#FFFFFF] p-6">
            <div className="flex items-center gap-2 mb-4">
              <span className="material-symbols-outlined text-[#111827]" style={{ fontSize: 16, fontVariationSettings: "'FILL' 1" }}>access_time</span>
              <span className="text-xs font-bold text-[#0F172A]">This week's best slot</span>
            </div>
            <div className="rounded-xl border border-[#111827]/20 bg-[#F5EBE8] p-5 mb-4">
              <p className="text-3xl font-semibold text-[#111827]">Tuesday</p>
              <p className="text-lg font-bold text-[#0F172A]">9:15 AM</p>
              <p className="text-xs text-[#64748B] mt-1">91% audience activity score</p>
            </div>
            <p className="text-xs text-[#64748B] leading-relaxed">Based on your last 30 days of follower data across Instagram. Updated every Sunday.</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 border-y border-[rgba(15,23,42,0.08)]">
        <div className="mx-auto max-w-4xl px-6">
          <div className="text-center mb-10">
            <p className="text-[0.68rem] font-bold uppercase tracking-[0.2em] text-[#111827] mb-3">Weekly plan</p>
            <h2 className="text-[clamp(24px,3.5vw,40px)] font-medium tracking-[-0.03em] text-[#0F172A]">Seven days of clarity.</h2>
            <p className="mt-3 text-[#64748B] max-w-md mx-auto text-sm leading-relaxed">Every Monday, Vielinks generates a focused action list based on last week's performance and upcoming opportunities.</p>
          </div>
          <div className="rounded-2xl border border-border bg-[#FFFFFF] divide-y divide-[rgba(15,23,42,0.06)]">
            {WEEKLY_ACTIONS.map((item, i) => (
              <div key={i} className="flex items-center gap-4 px-6 py-4">
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${item.done ? 'border-[#047857] bg-[#047857]' : 'border-[#CBD5E1]'}`}>
                  {item.done && (
                    <span className="material-symbols-outlined text-white" style={{ fontSize: 12, fontVariationSettings: "'FILL' 1" }}>check</span>
                  )}
                </div>
                <span className={`text-sm ${item.done ? 'text-[#94A3B8] line-through' : 'text-[#0F172A]'}`}>{item.label}</span>
                {!item.done && (
                  <span className="ml-auto text-[10px] font-bold text-[#111827] bg-[#111827]/10 px-2 py-0.5 rounded-full shrink-0">AI suggested</span>
                )}
              </div>
            ))}
          </div>
      ...
      <section className="py-16 mx-auto max-w-5xl px-6">
        <div className="text-center mb-10">
          <p className="text-[0.68rem] font-bold uppercase tracking-[0.2em] text-[#111827] mb-3">Guardrails</p>
          <h2 className="text-[clamp(24px,3.5vw,40px)] font-medium tracking-[-0.03em] text-[#0F172A]">AI that works for you, not around you.</h2>
          <p className="mt-3 text-[#64748B] max-w-md mx-auto text-sm leading-relaxed">Suggestions are a starting point. Your brand voice, review process, and judgment stay in control.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-4">
          {GUARDRAILS.map((g) => (
            <div key={g.title} className="rounded-2xl border border-border bg-[#FFFFFF] p-6">
              <div
              ...
              <h3 className="text-sm font-bold text-[#0F172A] mb-2">{g.title}</h3>
      ...
      <section className="py-20 mx-auto max-w-6xl px-6">
        <div className="text-center mb-14">
          <h2 className="text-[clamp(24px,3.5vw,40px)] font-medium tracking-[-0.03em] text-[#0F172A]">The full AI toolkit</h2>
          <p className="mt-3 text-[#64748B]">Every AI feature in one place — no prompt engineering required.</p>
        </div>
