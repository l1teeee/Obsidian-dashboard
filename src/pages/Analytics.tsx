    ...
    <div ref={pageRef}>
      ...
      <div className="p-8 space-y-8 max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {data.kpis.map((k) => (
            <div key={k.label} data-kpi className="glass-card p-6 rounded-3xl border border-[#15140F]/5 shadow-[0_0_40px_rgba(200,85,58,0.08)]">
              <div className="flex justify-between items-start mb-4">
                <span className="text-xs font-medium text-[#6B655B] uppercase tracking-widest">{k.label}</span>
                {k.badge
                  ? <span className={`px-2 py-1 text-[10px] font-bold rounded-md ${k.badgeColor === 'purple' ? 'bg-[#e4b9ff]/10 text-[#e4b9ff]' : 'bg-[#4F7A4A]/10 text-[#4F7A4A]'}`}>{k.badge}</span>
