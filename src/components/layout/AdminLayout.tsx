...
export default function AdminLayout() {
  ...
  const linkCls = (isActive: boolean) => [
    ...
    isActive
      ...
      : 'text-[#6A6470] hover:text-[#1C1814] hover:bg-white/[0.04] active:scale-[0.97]',
  ].join(' ');
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex">
      {/* Sidebar */}
      <aside className={[
        'fixed top-0 left-0 h-full flex flex-col bg-[#F4F0E8] border-r border-[#1C1814]/15 z-50 transition-all duration-300',
        open ? 'w-[220px]' : 'w-[56px]',
      ].join(' ')}>

        {/* Brand */}
        <div className={['flex items-center gap-2.5 py-5 border-b border-[#1C1814]/10', open ? 'px-4' : 'px-3 justify-center'].join(' ')}>
          ...
          {open && (
            <div className="overflow-hidden">
              <p className="text-xs font-extrabold text-[#1C1814] font-headline leading-tight">Vielinks</p>
        ...
        {/* Bottom */}
        <div className="p-2 border-t border-[#1C1814]/10 flex flex-col gap-0.5">
          <NavLink
            to="/dashboard"
          ...
          <div className="h-px bg-[#1C1814]/15 my-0.5" />

          <button
            onClick={() => setOpen(v => !v)}
            className={[linkCls(false), 'w-full text-left text-[#1C1814] hover:text-[#6A6470]'].join(' ')}
      ...
      <Modal open={logoutModal} onClose={() => setLogoutModal(false)} maxWidth="max-w-sm">
        <div className="p-8">
          ...
          <p className="text-sm text-[#6A6470] mb-7">You will be redirected to the login screen.</p>
          <div className="flex flex-col gap-2.5">
            <button
              onClick={() => { void confirmLogout(); }}
              className="w-full py-3 rounded-xl bg-[#f87171] text-[#1C1814] font-bold text-sm hover:bg-[#fca5a5] transition-all"
            >
              Yes, log out
            </button>
            <button
