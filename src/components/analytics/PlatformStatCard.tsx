...
export default function PlatformStatCard({ stat: p }: PlatformStatCardProps) {
  return (
    <div data-platform-stat className="glass-card p-6 rounded-3xl border border-[#1C1814]/5">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h4 className="font-bold text-sm text-[#1C1814]">{p.name}</h4>
          <p className="text-xs text-[#6A6470]">{p.handle}</p>
        </div>
      </div>
      <div className="space-y-4">
        <div className="flex justify-between">
          <span className="text-xs text-[#5C5650]">Follower Growth</span>
          <span className="text-xs font-mono text-[#7DD3C7]">{p.growthLabel}</span>
        </div>
        <div className="w-full h-1 bg-[#353534] rounded-full overflow-hidden">
          <div data-stat-bar className="h-full bg-[#7DD3C7] rounded-full" style={{ width: `${p.barPct}%` }} />
        </div>
        <div className="flex justify-between">
          <span className="text-xs text-[#5C5650]">Total Reach</span>
          <span className="text-xs font-mono text-[#1C1814]">{p.reach}</span>
        </div>
      </div>
    </div>
  );
