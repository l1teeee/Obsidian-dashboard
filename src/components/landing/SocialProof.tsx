...
function LogoPill({ name, platformId, color }: { name: string; platformId: string; color: string }) {
  return (
    <div className="flex shrink-0 items-center gap-2.5 rounded-xl border border-[rgba(15,23,42,0.14)] bg-[#FFFFFF] px-5 py-3 backdrop-blur-sm">
      <div
      ...
      <span className="text-[0.78rem] font-semibold text-[#64748B] tracking-tight">{name}</span>
    </div>
  );
}
...
export default function SocialProof() {
  ...
  return (
    <section
      ref={rootRef}
      style={{ opacity: 0 }}
      className="relative bg-[#F8FAFC] pt-16 pb-20 overflow-hidden"
    >
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#E2E8F0]/60 to-transparent" />
      ...
      <div className="relative z-10 mx-auto max-w-[1100px] px-6 md:px-12">
        <p
          data-sp="label"
          style={{ opacity: 0 }}
          className="mb-7 text-center text-[0.65rem] font-bold uppercase tracking-[0.22em] text-[#64748B]"
        >
      ...
      <div className="relative -mt-6 h-[200px] w-full overflow-hidden [mask-image:radial-gradient(55%_60%,white,transparent)]">
        {/* Subtle brand glow */}
        <div className="absolute inset-0 before:absolute before:inset-0 before:bg-[radial-gradient(circle_at_bottom_center,#111827,transparent_65%)] before:opacity-10" />
        {/* Curved horizon line */}
        <div className="absolute -left-1/2 top-1/2 z-10 aspect-[1/0.55] w-[200%] rounded-[100%] border-t border-[#E2E8F0] bg-[#F8FAFC]" />
      ...
      >
        <div className="flex flex-col gap-12 md:flex-row md:gap-0">
          {stats.map((s, i) => (
            ...
            >
              {i > 0 && (
                <div className="absolute left-0 top-1/2 hidden h-12 w-px -translate-y-1/2 bg-gradient-to-b from-transparent via-[#CBD5E1]/60 to-transparent md:block" />
              )}
              <div className="relative">
                <div className="absolute inset-0 -z-10 blur-[28px] opacity-20 bg-[#111827] rounded-full scale-75" />
                <span
                  ref={(el) => { numRefs.current[i] = el; }}
                  className="block text-[2.8rem] font-extrabold leading-none tracking-[-0.04em] text-[#0F172A] md:text-[3.2rem]"
                >
                  0{s.suffix}
                </span>
              </div>
              <span className="text-[0.85rem] font-semibold text-[#0F172A]/65">{s.label}</span>
              <span className="text-[0.68rem] tracking-wide text-[#64748B]">{s.sub}</span>
            </div>
          ))}
        </div>
      </div>
