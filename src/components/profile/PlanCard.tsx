...
export default function PlanCard() {
  ...
  if (!plan) {
    return (
      <div className="bg-[#F1F5F9] rounded-3xl border border-[#0F172A]/10 p-6 animate-pulse">
        <div className="h-4 w-20 bg-[#E2E8F0] rounded-full mb-3" />
        <div className="h-7 w-16 bg-[#E2E8F0] rounded-full mb-4" />
        <div className="space-y-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-3 bg-[#E2E8F0] rounded-full w-3/4" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      <div data-section className="bg-[#F1F5F9] rounded-3xl border border-[#0F172A]/10 overflow-hidden relative">
        {/* Ambient glow */}
        <div
        ...
        <div className="p-6 relative z-10">
          ...
          <div className="flex items-start justify-between mb-5">
            <div>
              <p className="text-[10px] text-[#64748B] uppercase tracking-widest mb-1">Current Plan</p>
              <h3 className="font-headline text-2xl font-extrabold text-[#0F172A] leading-none">{meta.label}</h3>
          ...
          {plan !== 'starter' && (
            <p className="text-[10px] text-[#64748B] mb-4">
              Renews <span className="text-[#0F172A] font-medium">Apr 25, 2026</span>
            </p>
          )}
