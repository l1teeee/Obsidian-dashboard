  ...
  return (
    <div className="w-full h-full bg-black flex flex-col pt-5">
      ...
      <div className="px-4 pb-2 flex justify-between items-center shrink-0">
        <span className="text-[#1C1814] font-bold text-base font-headline">Instagram</span>
        <div className="flex gap-4">
          <span className="material-symbols-outlined text-[#1C1814] text-[20px]">favorite</span>
          <span className="material-symbols-outlined text-[#1C1814] text-[20px]">send</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-yellow-400 via-red-500 to-purple-600 p-[1.5px] shrink-0">
            <div className="w-full h-full rounded-full bg-black flex items-center justify-center">
              <span className="text-[#7DD3C7] text-[11px] font-bold">
                {igAccountName ? igAccountName[0].toUpperCase() : 'I'}
              </span>
            </div>
          </div>
          <span className="text-xs font-bold text-[#1C1814] flex-1">
        ...
        {/* Media carousel */}
        <div className="w-full aspect-square bg-[#F0EBE2] relative overflow-hidden">
          {count > 0 ? (
            <>
              <img
                key={current}
              ...
              {count > 1 && (
                <div className="absolute top-2 right-2 bg-black/60 rounded-full px-2 py-0.5">
                  <span className="text-[10px] font-bold text-[#1C1814]">{current + 1}/{count}</span>
                </div>
              )}
              ...
              {count > 1 && current > 0 && (
                ...
                >
                  <span className="material-symbols-outlined text-[#1C1814] text-[16px]">chevron_left</span>
                </button>
              )}
              {count > 1 && current < count - 1 && (
                ...
                >
                  <span className="material-symbols-outlined text-[#1C1814] text-[16px]">chevron_right</span>
                </button>
              )}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 text-[#1C1814]">
        ...
        <div className="flex justify-between items-center px-3 py-2.5">
          <div className="flex gap-3">
            <span className="material-symbols-outlined text-[#1C1814] text-[20px]">favorite</span>
            <span className="material-symbols-outlined text-[#1C1814] text-[20px]">mode_comment</span>
            <span className="material-symbols-outlined text-[#1C1814] text-[20px]">send</span>
          </div>
          <span className="material-symbols-outlined text-[#1C1814] text-[20px]">bookmark</span>
        </div>

        {/* Caption */}
        <div className="px-3 pb-4 space-y-1">
          <p className="text-[11px] leading-relaxed text-[#1C1814]">
            <span className="font-bold">{igAccountName ?? 'your_account'} </span>
            <span className="text-[#5C5650]">{caption || 'Your caption will appear here...'}</span>
          </p>
          <p className="text-[9px] text-[#6A6470] uppercase font-medium">Just now</p>
        </div>
      </div>
      ...
      <div className="h-12 border-t border-[#353534] flex justify-around items-center bg-black shrink-0">
        {['home', 'search', 'add_box', 'video_library'].map(icon => (
          <span key={icon} className="material-symbols-outlined text-[#1C1814] text-[20px]">{icon}</span>
        ))}
