...
export default function PreviewPanel({
  ...
  return (
    <section className="w-full bg-[#F8FAFC] flex flex-col items-center justify-center p-4 md:p-8 relative overflow-hidden min-h-0">
      <div className="absolute -top-20 -right-20 w-80 h-80 bg-[#111827]/5 blur-[100px] rounded-full pointer-events-none" />
      <div className="absolute -bottom-20 -left-20 w-80 h-80 bg-[#D6A86A]/5 blur-[100px] rounded-full pointer-events-none" />

      {/* Platform tabs */}
      <div className="flex gap-1 p-1 bg-[#F1F5F9]/40 backdrop-blur-md rounded-full mb-8 border border-[#0F172A]/10 z-10">
      ...
      <div
        data-phone-mockup
        className="w-[300px] h-[590px] rounded-[3rem] border-[8px] border-[#CBD5E1] shadow-[0_40px_100px_rgba(0,0,0,0.6)] relative z-10 overflow-hidden"
      >
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-24 h-5 bg-[#CBD5E1] rounded-b-2xl z-20" />
      ...
      <div className="mt-6 flex items-center gap-2 px-4 py-2 rounded-full bg-[#F1F5F9]/50 border border-[#0F172A]/10 z-10">
        <span className="w-2 h-2 rounded-full bg-[#111827] animate-pulse" />
        <span className="text-[10px] font-bold uppercase tracking-widest text-[#64748B]">Real-time Preview</span>
        {mediaPreviews.length > 1 && (
          <span className="text-[10px] text-[#111827] font-bold">{mediaPreviews.length} images</span>
        )}
      </div>
    </section>
  );
